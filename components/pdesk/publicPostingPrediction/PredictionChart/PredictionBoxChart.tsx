...
import { MONTH_FOR_PREDICTION_ABSOLUTE_DELTA_CHART, PUBLIC_POSTING_MONTH_YEAR_IMPACT } from '../../../../url/pdesk';
import { get } from '../../../../util/servercall';
import AbsoluteDeltaChart from '../PlotlyPlot/AbsoluteDeltaChart';
import MetricPredictionBoxChartForm from '../PredictionForm/PredictionBoxChartForm';

interface Props {
  yVariable : any;    
}

const PredictionBoxChart:FC<Props> = ({ yVariable }) => {
  ...
  const fetchChartData = async () => {
    ...
    const absolute_delta_graph = await get(
      MONTH_FOR_PREDICTION_ABSOLUTE_DELTA_CHART +
        `?label=${YvariableValue}&month=${monthValue}&year=${yearValue}`
    );
    console.log(absolute_delta_graph.data.data)
  ...
  let publicPostingPredictionBoxPlot = [];

  publicPostingPredictionBoxPlot = absoluteDeltaGraph.map((data) => {
    return {
      y: data.y,
      type: 'box',
      hovertemplate:
        `lookahead number=%{x}<br>` +
        'abs_delta=%{y} <extra></extra>'
      };
  });
  ...
  return (
    <>
      ...
      <AbsoluteDeltaChart
        loading={isLoading}
        publicPostingBoxPlot={publicPostingPredictionBoxPlot}
...
export default PredictionBoxChart;
